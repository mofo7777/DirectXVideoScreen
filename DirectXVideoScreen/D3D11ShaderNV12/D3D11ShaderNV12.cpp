...
HRESULT CD3D11ShaderNV12::InitShaderNV12(CWICBitmap& cWICBitmap)
{
	...
	try
	{
		...
		IF_FAILED_THROW(InitD3D11Resources(cWICBitmap));
	}
	catch(HRESULT){}
...
HRESULT CD3D11ShaderNV12::ProcessShaderNV12(LPCWSTR wszOutputImageFile)
{
	HRESULT hr = S_OK;
	...
	IF_FAILED_RETURN(CreateBmpFileFromRgbaSurface(m_pViewTexture2D, wszOutputImageFile));
...
HRESULT CD3D11ShaderNV12::InitD3D11Resources(CWICBitmap& cWICBitmap)
{
	HRESULT hr = S_OK;

	IF_FAILED_RETURN(InitVertexPixelShader());
	...
	D3D11_VIEWPORT vp;
	vp.Width = (FLOAT)cWICBitmap.GetWidth();
	vp.Height = (FLOAT)cWICBitmap.GetHeight();
	vp.MinDepth = 0.0f;
	vp.MaxDepth = 1.0f;
	vp.TopLeftX = 0;
	vp.TopLeftY = 0;
...
HRESULT CD3D11ShaderNV12::InitVertexPixelShader()
{
	HRESULT hr = S_OK;
...
HRESULT CD3D11ShaderNV12::InitRenderTargetView(const UINT uiWidth, const UINT uiHeight)
{
	...
	IF_FAILED_RETURN(m_pD3D11Device->CreateTexture2D(&desc2D, NULL, &m_pViewTexture2D));
	IF_FAILED_RETURN(m_pD3D11Device->CreateRenderTargetView(m_pViewTexture2D, NULL, &m_pViewRT));
...
HRESULT CD3D11ShaderNV12::CreateBmpFileFromRgbaSurface(ID3D11Texture2D* pD3D11Texture2D, LPCWSTR wszOutputImageFile)
{
	HRESULT hr = S_OK;
	D3D11_TEXTURE2D_DESC desc2D;
	ID3D11Texture2D* pViewTexture = NULL;
	...
	try
	{
		pD3D11Texture2D->GetDesc(&desc2D);
		...
		IF_FAILED_THROW(m_pD3D11Device->CreateTexture2D(&desc2D, NULL, &pViewTexture));

		m_pD3D11DeviceContext->CopyResource(pViewTexture, pD3D11Texture2D);

		IF_FAILED_THROW(m_pD3D11DeviceContext->Map(pViewTexture, uiSubResource, D3D11_MAP_READ, 0, &MappedSubResource));
		...
		m_pD3D11DeviceContext->Unmap(pViewTexture, uiSubResource);
