...
HRESULT CD3D11ShaderNV12::ProcessShaderNV12(const UINT uiWidth, const UINT uiHeight, LPCWSTR wszOutputImageFile1, LPCWSTR wszOutputImageFile2, LPCWSTR wszOutputImageFile3, const enum SHADER_CONVERSION ShaderConversion)
{
	...
	else if(ShaderConversion == CONVERT_LUMACHROMACBCR_SHADER)
	{
		...
		IF_FAILED_RETURN(CreateBmpFileFromLumaSurface(m_pLumaRT, wszOutputImageFile1));
		IF_FAILED_RETURN(CreateBmpFileFromLumaSurface(m_pChromaBRT, wszOutputImageFile2));
		IF_FAILED_RETURN(CreateBmpFileFromLumaSurface(m_pChromaCRT, wszOutputImageFile3));
	}
	else if(ShaderConversion == CONVERT_CHROMADOWNSAMPLED_SHADER)
	{
		...
		{
			ProcessChromaDownSampledShader(m_pVertexChromaShader, m_pChromaDownSampledShader);
		}
	...
	else if(ShaderConversion == CONVERT_CHROMADOWNSAMPLED2_SHADER)
	{
		...
		{
			ProcessChromaDownSampledShader2(m_pVertexShader, m_pChromaDownSampledShader2);
		}

		IF_FAILED_RETURN(CreateBmpFileFromLumaSurface(m_pChromaBDownSampledRT, wszOutputImageFile1));
		IF_FAILED_RETURN(CreateBmpFileFromLumaSurface(m_pChromaCDownSampledRT, wszOutputImageFile2));
	}
	else if(ShaderConversion == CONVERT_FAKE_NV12_SHADER)
	{
		ProcessYCbCrShader2();
		InitViewPort(uiWidth, uiHeight + (uiHeight / 2));
		m_pD3D11DeviceContext->PSSetSamplers(0, 1, &m_pSamplerPointState);
		ProcessFakeNV12Shader();

		IF_FAILED_RETURN(CreateBmpFileFromNV12Surface(m_pFakeNV12RT, wszOutputImageFile1));
	}
	else
	{
...
void CD3D11ShaderNV12::OnRelease()
{
	...
	SAFE_RELEASE(m_pChromaBRT);
	SAFE_RELEASE(m_pChromaCRT);
	SAFE_RELEASE(m_pChromaDownSampledRT);
	SAFE_RELEASE(m_pChromaBDownSampledRT);
	SAFE_RELEASE(m_pChromaCDownSampledRT);
	...
	SAFE_RELEASE(m_pChromaBRSV);
	SAFE_RELEASE(m_pChromaCRSV);
	SAFE_RELEASE(m_pChromaDownSampledRSV);
	SAFE_RELEASE(m_pChromaBDownSampledRSV);
	SAFE_RELEASE(m_pChromaCDownSampledRSV);

	SAFE_RELEASE(m_pSamplerPointState);
	SAFE_RELEASE(m_pSamplerLinearState);
	SAFE_RELEASE(m_pVertexLayout);
	SAFE_RELEASE(m_pVertexShader);
	SAFE_RELEASE(m_pVertexChromaShader);
	...
	SAFE_RELEASE(m_pChromaDownSampledShader);
	SAFE_RELEASE(m_pChromaDownSampledShader2);
	SAFE_RELEASE(m_pCombinedYUVPixelShader);
...
void CD3D11ShaderNV12::ProcessYCbCrShader2()
{
	...
	pYCbCrRT[0] = m_pLumaRT;
	pYCbCrRT[1] = m_pChromaBRT;
	pYCbCrRT[2] = m_pChromaCRT;
...
void CD3D11ShaderNV12::ProcessChromaDownSampledShader2(ID3D11VertexShader* pVertexShader, ID3D11PixelShader* pPixelShader)
{
	...
	pChromaRT[0] = m_pChromaBDownSampledRT;
	pChromaRT[1] = m_pChromaCDownSampledRT;

	ID3D11ShaderResourceView* pChromaRSV[2];
	pChromaRSV[0] = m_pChromaBRSV;
	pChromaRSV[1] = m_pChromaCRSV;
...
void CD3D11ShaderNV12::ProcessFakeNV12Shader()
{
	ID3D11ShaderResourceView* pLumaChromaRSV[3];
	pLumaChromaRSV[0] = m_pChromaRSV;
	pLumaChromaRSV[1] = m_pChromaBRSV;
	pLumaChromaRSV[2] = m_pChromaCRSV;
	...
	m_pD3D11DeviceContext->VSSetShader(m_pVertexShader, NULL, 0);
	m_pD3D11DeviceContext->PSSetShader(m_pCombinedYUVPixelShader, NULL, 0);
	m_pD3D11DeviceContext->PSSetShaderResources(0, 3, pLumaChromaRSV);
	m_pD3D11DeviceContext->Draw(4, 0);
	m_pD3D11DeviceContext->Flush();
}

void CD3D11ShaderNV12::ProcessNV12Shader()
{
	// todo
}
...
HRESULT CD3D11ShaderNV12::InitVertexPixelShaders()
{
	...
	IF_FAILED_RETURN(InitVertexShaderFromFile(L"ChromaVS.hlsl", &m_pVertexChromaShader, TRUE));
	...
	IF_FAILED_RETURN(InitPixelShaderFromFile(L"ChromaDownSampledPS.hlsl", &m_pChromaDownSampledShader));
	//IF_FAILED_RETURN(InitPixelShaderFromFile(L"ChromaDownSampledPS2.hlsl", &m_pChromaDownSampledShader2));
	IF_FAILED_RETURN(InitPixelShaderFromFile(L"CombinedYUVPS.hlsl", &m_pCombinedYUVPixelShader));
...
HRESULT CD3D11ShaderNV12::InitRenderTargetChromaCb(const UINT uiWidth, const UINT uiHeight)
{
	...
	try
	{
		...
		IF_FAILED_THROW(m_pD3D11Device->CreateShaderResourceView(pTexture2D, &srvDesc2D, &m_pChromaBRSV));
		...
		IF_FAILED_THROW(m_pD3D11Device->CreateRenderTargetView(pTexture2D, &rtDesc, &m_pChromaBRT));
	}
	catch(HRESULT){}
...
HRESULT CD3D11ShaderNV12::InitRenderTargetChromaCr(const UINT uiWidth, const UINT uiHeight)
{
	...
	try
	{
		...
		IF_FAILED_THROW(m_pD3D11Device->CreateShaderResourceView(pTexture2D, &srvDesc2D, &m_pChromaCRSV));
		...
		IF_FAILED_THROW(m_pD3D11Device->CreateRenderTargetView(pTexture2D, &rtDesc, &m_pChromaCRT));
	}
	catch(HRESULT){}
...
HRESULT CD3D11ShaderNV12::InitRenderTargetChromaBDownSampled(const UINT uiWidth, const UINT uiHeight)
{
	...
	try
	{
		...
		IF_FAILED_THROW(m_pD3D11Device->CreateShaderResourceView(pTexture2D, &srvDesc2D, &m_pChromaBDownSampledRSV));
		...
		IF_FAILED_THROW(m_pD3D11Device->CreateRenderTargetView(pTexture2D, &rtDesc, &m_pChromaBDownSampledRT));
	}
	catch(HRESULT){}
...
HRESULT CD3D11ShaderNV12::InitRenderTargetChromaCDownSampled(const UINT uiWidth, const UINT uiHeight)
{
	...
	try
	{
		...
		IF_FAILED_THROW(m_pD3D11Device->CreateShaderResourceView(pTexture2D, &srvDesc2D, &m_pChromaCDownSampledRSV));
		...
		IF_FAILED_THROW(m_pD3D11Device->CreateRenderTargetView(pTexture2D, &rtDesc, &m_pChromaCDownSampledRT));
	}
	catch(HRESULT){}
