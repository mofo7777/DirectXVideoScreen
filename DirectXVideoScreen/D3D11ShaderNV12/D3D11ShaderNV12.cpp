...
HRESULT CD3D11ShaderNV12::ProcessShaderNV12(LPCWSTR wszOutputImageFile, const int iShader)
{
	HRESULT hr = S_OK;
	...
	if(iShader == CONVERT_INPUT_SHADER)
	{
		...
		IF_FAILED_RETURN(CreateBmpFileFromRgbaSurface(m_pViewRT, wszOutputImageFile));
	}
	else if(iShader == CONVERT_LUMA_SHADER)
	{
		...
		IF_FAILED_RETURN(CreateBmpFileFromLumaSurface(m_pLumaRT, wszOutputImageFile));
	}
	else
	{
