...
HRESULT CD3D11VideoProcessor::InitDXVA2(const UINT uiWidth, const UINT uiHeight, const DXGI_FORMAT dxgiFormat)
{
	HRESULT hr = S_OK;
...
HRESULT CD3D11VideoProcessor::CreateBmpFileFromYUVSurface(ID3D11Texture2D* pD3D11Texture2D, LPCWSTR wszOutputImageFile)
{
	...
	try
	{
		...
		m_pD3D11DeviceContext->CopyResource(pNV12Texture, pD3D11Texture2D);

		IF_FAILED_THROW(m_pD3D11DeviceContext->Map(pNV12Texture, uiSubResource, D3D11_MAP_READ, 0, &MappedSubResource));

		if(m_DXGIFormat == DXGI_FORMAT_NV12)
		{
		...
		else if(m_DXGIFormat == DXGI_FORMAT_YUY2)
		{
			hr = ProcessYUY2ToBmpFile(wszOutputImageFile, (BYTE*)MappedSubResource.pData, MappedSubResource.RowPitch, desc2D.Width, desc2D.Height);
		}
