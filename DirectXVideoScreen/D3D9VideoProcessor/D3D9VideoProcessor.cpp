...
HRESULT CD3D9VideoProcessor::InitDXVA2(const HWND hWnd, const UINT uiWidth, const UINT uiHeight, const D3DFORMAT format)
{
	HRESULT hr = S_OK;
...
HRESULT CD3D9VideoProcessor::ProcessImageConversion(LPCWSTR wszImageFile, LPCWSTR wszOutputImageFile)
{
	...
	try
	{
		...
		IF_FAILED_THROW(m_pDXVAHDDevice->CreateVideoSurface(m_uiWidth, m_uiHeight, m_D3DFormat, D3DPOOL_DEFAULT, 0, DXVAHD_SURFACE_TYPE_VIDEO_OUTPUT, 1, &pOutputSurface, NULL));
		...
		IF_FAILED_THROW(CreateBmpFileFromYUVSurface(pOutputSurface, wszOutputImageFile));
	}
	catch(HRESULT){}
...
HRESULT CD3D9VideoProcessor::InitVideoProcessor(const UINT uiWidth, const UINT uiHeight)
{
	...
	try
	{
		...
		IF_FAILED_THROW(CheckDxva2Format(m_D3DFormat, D3DFMT_A8R8G8B8, caps.OutputFormatCount, caps.InputFormatCount));
...
HRESULT CD3D9VideoProcessor::ConfigureVideoProcessor(const D3DFORMAT d3dFmt, const UINT uiWidth, const UINT uiHeight)
{
	...
	*/
	...
	//const DXVAHD_STREAM_STATE_INPUT_COLOR_SPACE_DATA Color = {0, 0, 0, 0};
...
HRESULT CD3D9VideoProcessor::CreateBmpFileFromYUVSurface(IDirect3DSurface9* pOutputSurface, LPCWSTR wszOutputImageFile)
{
	HRESULT hr = S_OK;
	D3DSURFACE_DESC desc;
	D3DLOCKED_RECT d3dLock;

	IF_FAILED_RETURN((m_D3DFormat != D3DFMT_YUY2 && m_D3DFormat != (D3DFORMAT)D3DFMT_NV12) ? E_UNEXPECTED : S_OK);

	IF_FAILED_RETURN(pOutputSurface->GetDesc(&desc));

	IF_FAILED_RETURN(pOutputSurface->LockRect(&d3dLock, NULL, D3DLOCK_READONLY));

	if(m_D3DFormat == (D3DFORMAT)D3DFMT_NV12)
	{
		hr = ProcessNV12ToBmpFile(wszOutputImageFile, (BYTE*)d3dLock.pBits, d3dLock.Pitch, desc.Width, desc.Height);
	}
	else if(m_D3DFormat == D3DFMT_YUY2)
	{
		hr = ProcessYUY2ToBmpFile(wszOutputImageFile, (BYTE*)d3dLock.pBits, d3dLock.Pitch, desc.Width, desc.Height);
	}
